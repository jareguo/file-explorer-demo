...
function Tree(jquery_element) {
    ...
    jquery_element.jstree({                     // create instance
        'core' : {
            ...
            'themes' : {
                'dots' : false,
                'responsive' : false,       // always small icon
            }
        }
    });
    ...
    var self = this;
    jquery_element.on('changed.jstree', function (e, data) {    // we don't use select.jstree cause it can't be suppressed
...
Tree.prototype.navigate = function (dir) {
    ...
    for (var i = 0; i < pathes.length; ++i) {
        ...
        if (win32 && nodePath == win32.MY_COMPUTER_PATH) {
            ...
            this._createFolders(parsingNode, nodePath, ignoreNavigateItem);
        }
    }
...
Tree.prototype.createDrives = function (parentNode, ignoreItem) {
    var self = this;
    $.get(SERVER_URL + "get_drives")
        .done(function (data) {
            var drives = data.files;
            for (var i = 0; i < drives.length; ++i) {
                var driveName = drives[i].name;
...
Tree.prototype._createFolders = function (parentNode, dir, ignoreItem) {
    var self = this;
    dir += path.sep;
    $.get(SERVER_URL + "navigate", { path : dir })
        .done(function (data) {
            var itemList = data.files;
            for (var i = 0; i < itemList.length; ++i) {
                var item = itemList[i];
                if (item.name != ignoreItem && item.type == 'folder') {
                    var path = dir + item.name;
                    self.jstree.create_node(parentNode, { text : item.name, data : path }, "inside");
                }
            }
            self.jstree.open_node(parentNode, null, false);
        })
        .fail(function () {
            console.error('failed to open: ' + dir);
        });
}
...
if (win32) {
    Tree.prototype.selectDrive = function () {
        ...
        var myComputer = this.jstree.find_node(this.jstree.get_node(this.element), win32.MY_COMPUTER_NAME);
        if (myComputer) {
            this.jstree.deselect_all(true);
