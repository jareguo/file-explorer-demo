...
AddressBar.prototype.set = function (dirPath) {
    ...
    for (var i = 0; i < sequence.length; ++i) {
        result.push({
            name: sequence[i],
            path: sequence.slice(0, 1 + i).join(path.sep) + path.sep,
        });
    }
...
AddressBar.prototype.enter = function (dir) {
    ...
    if (where == how_many - 1) {
        // Add '/' on tail
        this.element.children().eq(-1).append('<span class="divider">' + path.sep + '</span>');
    }
    else {
    ...
    if (!dirName) {
        dirName = dir;  // 如果dir是‘D:’，则basename返回空字符串
    }
    this.element.append(gen_one_fie({dir: dir, name: dirName}));

    // update style and folder view
